...
def build_output(data):
    ...
    text = (
        f"󰚩 "
        f"<span color='#6c7086'>5h</span> "
        f"<span color='{five_color}'>{five_label}</span>  "
        f"<span color='#6c7086'>7d</span> "
        f"<span color='{seven_color}'>{seven_label}</span>"
    )

    # --- TOOLTIP ---
    tt = "<b><span color='#cba6f7'>╔══════ CLAUDE CODE USAGE ═══════╗</span></b>\n"

    tt += f"<b><span color='#cba6f7'>║</span></b> <b><span color='#89dceb'>SESSION</span></b> <span color='#45475a'>(5h)</span>  <span color='#6c7086'>resets in {five_reset}</span>\n"
    ...
    tt += f"<b><span color='#cba6f7'>║</span></b> <b><span color='#89b4fa'>WEEKLY</span></b> <span color='#45475a'>(7d)</span>  <span color='#6c7086'>resets in {seven_reset}</span>\n"
    ...
    if extra.get("is_enabled"):
        ...
        tt += f"<b><span color='#cba6f7'>║</span></b> <span color='#45475a'>[{get_progress_bar(extra_pct)}]</span> <span color='{extra_color}'><b>{extra_pct}%</b></span>  <span color='#6c7086'>${used:.2f} / ${limit:.2f}</span>\n"
